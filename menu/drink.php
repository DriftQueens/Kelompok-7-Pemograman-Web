<?php

session_start();

if (!isset($_SESSION['username'])) {
    header('location:index.php');
}

include("config.php");
$_SESSION['pesan'] = 'drink';
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Restop.</title>

    <link rel="stylesheet" href="https://unpkg.com/swiper/swiper-bundle.min.css" />

    <!-- font awesome cdn link  -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.3/css/all.min.css">

    <!-- custom css file link  -->
    <link rel="stylesheet" href="style.css">
    
</head>
<body class="bek">
    <!-- header -->
    <header>

        <a href="#" class="logo"><i class="fa-sharp fa-solid fa-champagne-glass"></i>restop.</a>


        <div class="icons">
            <i class="fas fa-bars" id="menu-bars"></i>
            <i class="fas fa-search" id="search-icon"></i>
            <a href="profile.php" class="fas fa-user"></a>
            <a href="home.html" class="fas fa-home"></a>
            <a href="listorder.php" class="fas fa-cart"></a>
        </div>
    
    </header>
    
    <!-- header section ends-->
    
    <!-- search form  -->
    
    <form action="" id="search-form">
        <input type="search" placeholder="search here..." name="" id="search-box">
        <label for="search-box" class="fas fa-search"></label>
        <i class="fas fa-times" id="close"></i>
    </form>
    
    <!--End Search Form-->
    <!--menu-->
    <section class="menu" id="menu">
        <div style="margin-top: 70px;">
            <h3 class="sub-heading"> Menu Kami </h3>
            <h1 class="heading"> Khusus Hari Ini </h1>
        </div>

    
        <div class="box-container">
    
            <div class="box">
                <div class="image">
                    <img src="images/cappucino.jpg" alt="">
                    <a href="#" class="fas fa-heart"></a>
                </div>
                <div class="content">
                    <div class="stars">
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star-half-alt"></i>
                    </div>
                    <h3>Cappucino</h3>
                    <p>Cocok disajikan dengan berbagai macam jenis makanan.</p>
                    <a href="order.html" class="btn">Pesan</a>
                    <span class="price">Rp. 35.000</span>
                </div>
            </div>
    
            <div class="box">
                <div class="image">
                    <img src="images/choco.jpg" alt="">
                    <a href="#" class="fas fa-heart"></a>
                </div>
                <div class="content">
                    <div class="stars">
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star-half-alt"></i>
                    </div>
                    <h3>Coffe Choco</h3>
                    <p>Cocok untuk anda yang pencinta manis.</p>
                    <a href="order.html" class="btn">Pesan</a>
                    <span class="price">Rp. 45.000</span>
                </div>
            </div>
    
            <div class="box">
                <div class="image">
                    <img src="images/smoothie.jpg" alt="">
                    <a href="#" class="fas fa-heart"></a>
                </div>
                <div class="content">
                    <div class="stars">
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star-half-alt"></i>
                    </div>
                    <h3>Rasberry Smoothie</h3>
                    <p>Mempertahankan tekstur Rasberry yang masih khas</p>
                    <a href="order.html" class="btn">Pesan</a>
                    <span class="price">Rp. 30.000</span>
                </div>
            </div>
    
            <div class="box">
                <form action="" method="post">
                <div class="image">
                    <img src="images/tea.jpg" alt="">
                    <a href="#" class="fas fa-heart"></a>
                </div>
                <div class="content">
                    <div class="stars">
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star-half-alt"></i>
                    </div>
                    <h3>Tea</h3>
                    <p>Cocok disajikan dengan berbagai macam jenis makanan.</p>
                    <?php
                        if (isset($_POST['submit'])) {

                            $hasil = mysqli_query($conn, "INSERT INTO orderlist (pesanan, harga) VALUES ('Tea', '29000')");
                            header('location:drink.php');
                        }
                        ?>
                    <button type="submit" class="btn" name="submit" value="Add">Add to cart</button>
                    <span class="price">Rp. 29.000</span>
                </div>
                </form>
            </div>
    
            <div class="box">
                <div class="image">
                    <img src="images/wine.jpg" alt="">
                    <a href="#" class="fas fa-heart"></a>
                </div>
                <div class="content">
                    <div class="stars">
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star-half-alt"></i>
                    </div>
                    <h3>Wine</h3>
                    <p>Favorit</p>
                    <a href="order.html" class="btn">Pesan</a>
                    <span class="price">Rp. 60.000</span>
                </div>
            </div>
    
            <div class="box">
                <div class="image">
                    <img src="images/margarita.jpg" alt="">
                    <a href="#" class="fas fa-heart"></a>
                </div>
                <div class="content">
                    <div class="stars">
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star-half-alt"></i>
                    </div>
                    <h3>Margarita</h3>
                    <p>Perfect Drink</p>
                    <a href="order.html" class="btn">Pesan</a>
                    <span class="price">Rp. 60.000</span>
                </div>
            </div>
    
            <div class="box">
                <div class="image">
                    <img src="images/menu-7.jpg" alt="">
                    <a href="#" class="fas fa-heart"></a>
                </div>
                <div class="content">
                    <div class="stars">
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star-half-alt"></i>
                    </div>
                    <h3>Justie</h3>
                    <p>Perpaduan antara buah dan tea</p>
                    <a href="order.html" class="btn">add to cart</a>
                    <span class="price">Rp. 23.000</span>
                </div>
            </div>
    
            <div class="box">
                <div class="image">
                    <img src="images/nipis.jpg" alt="">
                    <a href="#" class="fas fa-heart"></a>
                </div>
                <div class="content">
                    <div class="stars">
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star-half-alt"></i>
                    </div>
                    <h3>Jeruk Nipis</h3>
                    <p>Sangat segar, menghilangi dahaga.</p>
                    <a href="order.html" class="btn">Pesan</a>
                    <span class="price">Rp. 35.000</span>
                </div>
            </div>
    
            <div class="box">
                <div class="image">
                    <img src="images/menu-9.jpg" alt="">
                    <a href="#" class="fas fa-heart"></a>
                </div>
                <div class="content">
                    <div class="stars">
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star-half-alt"></i>
                    </div>
                    <h3>Orange Juice</h3>
                    <p>Melepas dahaga</p>
                    <a href="order.html" class="btn">Pesan</a>
                    <span class="price">Rp. 25.000</span>
                </div>
            </div>
    
        </div>
    
    </section>
<!--End Menu-->    

<body>

<script src="https://unpkg.com/swiper/swiper-bundle.min.js"></script>

<!-- custom js file link  -->
<script src="script.js"></script>
</body>
</html>